...
from common.metrics import visualcomet_metrics
...
def train_net(args, config):
    ...
    if config.MODULE == VisualCometEventOrdering:
        train_metrics_list.append(visualcomet_metrics.EventAccuracy(**metric_kwargs))
        val_metrics_list.append(visualcomet_metrics.EventAccuracy(**metric_kwargs))
    else:
        train_metrics_list.append(visualcomet_metrics.ImageAccuracy(**metric_kwargs))
        val_metrics_list.append(visualcomet_metrics.ImageAccuracy(**metric_kwargs))
        val_metrics_list.append(visualcomet_metrics.ImageEdit(**metric_kwargs))

    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(visualcomet_metrics.LossLogger(output_name, display_name=display_name, **metric_kwargs))
        val_metrics_list.append(visualcomet_metrics.LossLogger(output_name, display_name=display_name, **metric_kwargs))
    ...
    host_metric_name = 'EVEAcc' if config.MODULE == VisualCometEventOrdering else 'IOCEdit'
