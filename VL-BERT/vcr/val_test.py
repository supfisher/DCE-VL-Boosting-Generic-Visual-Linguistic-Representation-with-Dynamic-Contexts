...
def parse_args():
    ...
    parser.add_argument('--result-path', type=str, default='./vcr_test_results')
    parser.add_argument('--result-name', type=str, default='vl-bert')
    parser.add_argument('--use-cache', default=False, action='store_true')
    parser.add_argument('--annot', type=str, default='./data/vcr/test.jsonl')
...
def main():
    ...
    if args.use_cache:
        ...
        if a_config is not None and args.a_ckpt is not None:
            ...
            # get model
            device_ids = [int(k) for k in args.gpus.split(',')]
            # os.environ['CUDA_VISIBLE_DEVICES'] = ','.join([str(k) for k in args.gpus])
            a_config.GPUS = ','.join([str(k) for k in args.gpus.split(',')])
            ...
            answer_loader = make_dataloader(a_config, mode='test', distributed=False)
        ...
        if r_config is not None and args.r_ckpt is not None:
            ...
            device_ids = [int(k) for k in args.gpus.split(',')]
            # os.environ['CUDA_VISIBLE_DEVICES'] = ','.join([str(k) for k in args.gpus])
            r_config.GPUS = ','.join([str(k) for k in args.gpus.split(',')])
            ...
            rationale_loader = make_dataloader(r_config, mode='test', distributed=False)
            ...
            # for r_batch in rationale_loader:
                ...
                r_out, _ = rationale_model(*r_batch)
