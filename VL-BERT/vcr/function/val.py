...
#         batch = to_cuda(batch)
#         if type(label_index_in_batch) is list:
#             datas = [batch[i] for i in range(len(batch)) if i not in label_index_in_batch]
#             outputs = net.inference_forward(*datas)
#             outputs.update({'answer_label': batch[label_index_in_batch[0]]})
#             outputs.update({'rationale_label': batch[label_index_in_batch[1]]})
