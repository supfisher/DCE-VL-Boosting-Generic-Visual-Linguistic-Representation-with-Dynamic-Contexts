...
def train_net(args, config):
    ...
    # metrics
    if config.DATASET.TASK == 'Q2AR':
        train_metrics_list = [vcr_metrics.JointAccuracy(allreduce=args.dist,
                                                   num_replicas=world_size if args.dist else 1)]
        val_metrics_list = [vcr_metrics.JointAccuracy(allreduce=args.dist,
                                                 num_replicas=world_size if args.dist else 1)]
    else:
