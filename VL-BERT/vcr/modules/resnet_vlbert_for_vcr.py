    ...
    def __init__(self, config):
        ...
        if not self.for_pretrain:
            ...
            if config.NETWORK.CLASSIFIER_TYPE == "2fc":
                ...
                # self.final_mlp_rationale = torch.nn.Sequential(
                #     torch.nn.Dropout(config.NETWORK.CLASSIFIER_DROPOUT, inplace=False),
                #     torch.nn.Linear(dim, config.NETWORK.CLASSIFIER_HIDDEN_SIZE),
                #     torch.nn.ReLU(inplace=True),
                #     torch.nn.Dropout(config.NETWORK.CLASSIFIER_DROPOUT, inplace=False),
...
class RationaleResNetVLBERT(Module):
    def __init__(self, config):
        super(RationaleResNetVLBERT, self).__init__(config)
        self.answer_bert = ResNetVLBERT(config)
        self.rational_bert = ResNetVLBERT(config)
    ...
    def forward(self,
                      ...
                      answer_label,
                      im_info,
                      rationale_choices=None,
                      rationale_align_matrix=None,
                      rationale_label=None,
                      mask_position=None,
                      mask_type=None,
                      mask_label=None):

        outputs, loss = self.answer_bert.forward(image,
                                                boxes,
                                                masks,
                                                question,
                                                question_align_matrix,
        ...
        if rationale_choices is not None:
            tmp_answer_choices = torch.stack([answer_choices[i, ind] for i, ind in enumerate(outputs['label_logits'].argmax(dim=-1))], dim=0)

            # tmp_answer_choices = torch.cat([answer_choices[:, i] for i in range(4)], dim=1)
            question = torch.cat([question, tmp_answer_choices], dim=1)

            rationale_outputs, rationale_loss = self.rational_bert.one_train_forward(image,
                                                boxes,
                                                masks,
                                                question,
                                                question_align_matrix,
                                                rationale_choices,
                                                rationale_align_matrix,
                                                rationale_label,
                                                im_info,
                                                mask_position=mask_position,
                                                mask_type=mask_type,
            ...
            loss = loss + rationale_loss
            outputs['ans_loss'] = outputs['ans_loss'] + rationale_outputs['ans_loss']
            if 'cnn_regularization_loss' in outputs:
                outputs['cnn_regularization_loss'] = outputs['cnn_regularization_loss'] + rationale_outputs['cnn_regularization_loss']

            outputs.update({
                'answer_label_logits': outputs['label_logits'],
                'answer_label': outputs['label'],
                'rationale_label_logits': rationale_outputs['label_logits'],
                'rationale_label': rationale_outputs['label']
            })
    ...
    def inference_forward(self,
        ...
        outputs = self.answer_bert.inference_forward(image,
                                             boxes,
                                             masks,
                                             question,
                                             question_align_matrix,
        ...
        if rationale_choices is not None:
            tmp_answer_choices = torch.stack(
                [answer_choices[i, ind] for i, ind in enumerate(outputs['label_logits'].argmax(dim=-1))], dim=0)

            # tmp_answer_choices = torch.cat([answer_choices[:, i] for i in range(4)], dim=1)
            question = torch.cat([question, tmp_answer_choices], dim=1)

            rationale_outputs = self.rational_bert.one_inference_forward(image,
                                                           boxes,
                                                           masks,
                                                           question,
                                                           question_align_matrix,
