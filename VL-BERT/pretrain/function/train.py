...
def train_net(args, config):
    ...
    if config.NETWORK.WITH_CONTRASTIVE_LOSS:
        train_metrics_list.append(pretrain_metrics.ContrastiveSimilarity(**metric_kwargs))
        val_metrics_list.append(pretrain_metrics.ContrastiveSimilarity(**metric_kwargs))
    ...
    if "multitask" in config.MODEL_PREFIX:
        host_metric_name = 'MLMAccWVC'
    else:
        host_metric_name = 'MLMAcc'

    # host_metric_name = 'ContrastiveSimilarity'
