...
class COCOCaptionsDataset(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, seq_len=64,
        ...
        annot = {'train': 'annotations/captions_train2014.json',
                 'val': 'annotations/captions_val2014.json'}
        annot_inst = {'train': 'annotations/instances_train2014.json',
                      'val': 'annotations/instances_val2014.json'}
        if zip_mode:
            self.root = os.path.join(data_path, '{0}2014.zip@/{0}2014'.format(image_set))
        else:
            self.root = os.path.join(data_path, '{}2014'.format(image_set))
    ...
    def __getitem__(self, index):
        ...
        rand_index = random.randrange(0, len(self.ids))
        # if self.with_contrastive_task:
        return self.load_imgs(index, relationship_label) + self.load_imgs(rand_index, relationship_label)
    ...
    def load_imgs(self, index, relationship_label=1):
        ...
        if relationship_label == 1:
            ...
            anns = self.coco.loadAnns(self.coco.getAnnIds(imgIds=rand_img_id))
