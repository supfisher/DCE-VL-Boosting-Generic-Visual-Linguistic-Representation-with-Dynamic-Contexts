...
class EventContrastiveLoss(nn.Module):
    ...
    def forward(self, emb_i, emb_j, labels):
        ...
        norm = emb_i.matmul(self.encoder.bilinear.weight.data.squeeze().T).norm(dim=1)
        norm_T = emb_i.matmul(self.encoder.bilinear.weight.data.squeeze()).norm(dim=1)
        scores, loss = self.encoder.pair_loss(emb_i, emb_j, self.temperature, labels)
        # loss += self.encoder.bilinear.weight.norm()*(1e-2)
