...
from data import (TokenBucketSampler,
                  ...
                  MlmDataset, mlm_collate, ConDataset, con_collate, MrfrDataset, mrfr_collate,
                  TxtTokLmdb)
...
def build_mlm_dataset_vcr(txt_db, img_db_gt, img_db, is_train, opts):
    if is_train:
...
def build_mrfr_dataset_vcr(txt_db, img_db_gt, img_db, is_train, opts):
    if is_train:
...
def build_mrc_dataset_vcr(txt_db, img_db_gt, img_db, is_train, opts):
    if is_train:
            ...
            for vcr_task in ["qa", "qar"]:
                ...
                for i, t in enumerate(dset['tasks']):
                    ...
                    if task.startswith('mlm'):
                        dataset = build_mlm_dataset_vcr(
                            txt_db, img_db_gt, img_db, is_train, opts)
                    elif task.startswith('mrfr'):
                        dataset = build_mrfr_dataset_vcr(
                            txt_db, img_db_gt, img_db, is_train, opts)
                    elif task.startswith('mrc'):
                        dataset = build_mrc_dataset_vcr(
                            txt_db, img_db_gt, img_db, is_train, opts)
                    else:
