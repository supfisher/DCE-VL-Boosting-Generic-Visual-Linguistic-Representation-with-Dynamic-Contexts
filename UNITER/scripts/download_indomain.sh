...
if [ ! -d $DOWNLOAD/img_db/vg ] ; then
    wget -c $BLOB/img_db/vg.tar -P $DOWNLOAD/img_db/
    tar -xvf $DOWNLOAD/img_db/vg.tar -C $DOWNLOAD/img_db
fi

# text dbs
for SPLIT in 'train' 'restval' 'val'; do
    wget -c $BLOB/txt_db/pretrain_coco_$SPLIT.db.tar -P $DOWNLOAD/txt_db/
#    tar -xvf $DOWNLOAD/txt_db/pretrain_coco_$SPLIT.db.tar -C $DOWNLOAD/txt_db
done
for SPLIT in 'train' 'val'; do
#    wget -c $BLOB/txt_db/pretrain_vg_$SPLIT.db.tar -P $DOWNLOAD/txt_db/
