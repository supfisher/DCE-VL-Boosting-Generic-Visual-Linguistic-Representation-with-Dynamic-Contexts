...
if __name__ == '__main__':
    ...
    # img2metadata = {}
    # len_e = 0
    # for annotation in ['train_annots.json', 'val_annots.json', 'test_annots.json']:
    #     with open(os.path.join(args.annotations, annotation), 'r') as f:
    #         examples = json.load(f)
    #     for example in examples:
    #         img_fn = example['img_fn'].split('/')[-1] if '/' in example['img_fn'] else example['img_fn']
    #         if img_fn in img2metadata:
    #             continue
    #         img2metadata[img_fn] = example['metadata_fn']

    feature_files = []
    img2metadata = {}
    parse_record_cache = os.path.join(record_cache, '{}_record_img_event.txt'.format(args.split))
    with open(parse_record_cache, 'r+', encoding="utf-8") as ann:
        for id_, l in enumerate(ann.readlines()):
            img_fn, metadata_fn, _ = l.split(",")
            img_fn = '.'.join(basename(img_fn).split('.')[:-1])
            img2metadata[img_fn+'.jpg'] = metadata_fn
