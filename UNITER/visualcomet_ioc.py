...
from data import (TokenBucketSampler, TokenBucketSamplerForItm,
                  ...
                  ConDataset, con_collate, EocDataset,
                  IocDataset, ioc_collate, IocDatasetWOCaption, ioc_collate_wo_cap)

from model.visualcomet import UniterVisualComet
...
def build_dataloader(dataset, collate_fn, is_train, opts):
    ...
    loader = DataLoader(dataset, batch_sampler=sampler,
                        num_workers=0, pin_memory=opts.pin_mem,
...
def build_con_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        collate_fn = con_collate
        datasets = [ConDataset(t, i, is_train=is_train) for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        collate_fn = con_collate
        dataset = ConDataset(txt_db, img_db, is_train=is_train)
...
def build_mrfr_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [IocDataset(t, i, is_train, opts.mrm_prob)
                    for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        dataset = IocDataset(txt_db, img_db, is_train, opts.mrm_prob)

    return dataset, ioc_collate
...
def build_eoc_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [EocDataset(t, i, opts.itm_neg_prob)
                    for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        dataset = EocDataset(txt_db, img_db, opts.itm_neg_prob)
    collate_fn = itm_collate
    return dataset, collate_fn


def build_ioc_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [IocDataset(t, i, is_train, opts.itm_neg_prob)
                    for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        dataset = IocDataset(txt_db, img_db, is_train, opts.itm_neg_prob)
    collate_fn = ioc_collate
    return dataset, collate_fn


def build_ioc_wocap_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [IocDatasetWOCaption(t, i, is_train, opts.itm_neg_prob)
                    for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
        dataset = IocDatasetWOCaption(txt_db, img_db, is_train, opts.itm_neg_prob)
    collate_fn = ioc_collate_wo_cap
    return dataset, collate_fn
    ...
    for dset in datasets:
        ...
        for i, t in enumerate(dset['tasks']):
            ...
            if task.startswith('mlm'):
                ...
                dataset = build_con_dataset(txt_db, img_db, is_train, opts)
            elif task.startswith('eoc'):
                dataset = build_eoc_dataset(txt_db, img_db, is_train, opts)
            elif task.startswith('ioc'):
                dataset = build_ioc_dataset(txt_db, img_db, is_train, opts)
            elif task.startswith('wocap'):
                dataset = build_ioc_wocap_dataset(txt_db, img_db, is_train, opts)
            else:
...
def main(opts):
    ...
    # train_val_dataloaders, all_img_dbs = create_dataloaders(
    #     opts.train_datasets, True, opts)
    ...
    global_step = opts.begin_global_step
    ...
    for step, (name, batch) in enumerate(meta_loader):
        ...
        if (step + 1) % opts.gradient_accumulation_steps == 0:
            ...
            if global_step % 100 == 0:
                ...
                print("Traing Loss: ", task2loss[name].val)
            ...
            if global_step % opts.valid_steps == 0:
                ...
                # validate(model, train_val_dataloaders)
...
def validate(model, val_dataloaders):
    model.eval()
    mean_acc, mean_loss, sum_word = 0, 0, 0
    for task, loader in val_dataloaders.items():
        LOGGER.info(f"validate on {task} task")
        if task.startswith('con'):
            val_log = validate_con(model, loader)
        elif task.startswith('mlm'):
            val_log = validate_mlm(model, loader)
            mean_acc += val_log['n_correct']
            mean_loss += val_log['loss']
            sum_word += val_log['n_word']
        elif task.startswith('mrfr'):
            ...
            val_log = validate_eoc(model, loader)
        elif task.startswith('ioc'):
            val_log = edit_distance_ioc(model, loader)
        elif task.startswith('wocap'):
            val_log = edit_distance_ioc_wocap(model, loader)
        else:
            raise ValueError(f'Undefined task {task}')
    ...
    if sum_word != 0:
        mean_acc /= sum_word
        mean_loss /= sum_word
        LOGGER.info(f"Average MLM accuracy: {mean_acc*100:.2f} "
                    f"Average MLM loss: {mean_loss:.2f}")
    model.train()
...
@torch.no_grad()
def validate_con(model, val_loader):
    LOGGER.info("start running CON validation...")
    val_loss = 0
    n_correct = 0
    n_word = 0
    st = time()
    for i, batch in enumerate(val_loader):
        loss, similarity_matrix = model(batch, task='con', compute_loss=False)
        if loss.sum().item() != float('-inf'):
            val_loss += loss.sum().item()
        pred_idx = similarity_matrix.argmax(dim=1)
        n_correct += (pred_idx == torch.arange(similarity_matrix.shape[0]).to(pred_idx.device)).sum().item()
        n_word += similarity_matrix.shape[0]
    val_loss = sum(all_gather_list(val_loss))
    n_correct = sum(all_gather_list(n_correct))
    n_word = sum(all_gather_list(n_word))
    tot_time = time()-st
...
@torch.no_grad()
def validate_ioc(model, val_loader):
    LOGGER.info("start running IOC validation...")
    val_loss = 0
    n_correct = 0
    n_word = 0
    st = time()
    for i, batch in enumerate(val_loader):
        scores = model(batch, task='ioc', compute_loss=False)
        labels = batch['ordering']
        labels = labels[labels != -1]
        loss = F.cross_entropy(scores, labels, reduction='sum')
        val_loss += loss.item()
        n_correct += (scores.max(dim=-1)[1] == labels).sum().item()
    ...
    LOGGER.info(f"validation finished in {int(tot_time)} seconds, "
                f"acc: {acc*100:.2f}")
    val_dist_log = edit_distance_ioc(model, val_loader)
    val_log.update(val_dist_log)
    return val_log
...
def label2ordering(labels, len_ordering):
    ...
    return ordering
# def label2ordering(ioc_scores, len_ordering, mean_score):
#     ordering = list(range(len_ordering))
#     for i in range(len_ordering):
#         "check how many 1s are for a specific index"
#         positive = (ioc_scores[i]>0.5).sum()
#         ordering[i] = len_ordering - positive - 1
#     return ordering


@torch.no_grad()
def edit_distance_ioc(model, val_loader):
    LOGGER.info("start running IOC edit distance validation...")
    edit_dist = 0
    n_word = 0
    n_label = 0
    n_acc = 0
    ...
    for i, samples in enumerate(dataset.image_ordering_one_cluster):
        ...
        ioc_scores = model(batch, task='ioc', compute_loss=False)
        pred_labels = ioc_scores.new_zeros(ioc_scores.shape)
        pred_labels[ioc_scores>0] = 1
        pred_ordering = label2ordering(pred_labels.squeeze().int(), len(ordering))
        edit_dist += levenshtein(''.join([str(i) for i in range(len(ordering))]), ''.join([str(i) for i in pred_ordering]))
        n_word += len(ordering)
        ordering = batch['ordering']
        n_acc += sum(ordering==pred_labels.squeeze().int()).item()
        n_label += len(ordering)
    edit_dist = sum(all_gather_list(edit_dist))
    n_word = sum(all_gather_list(n_word))
    n_acc = sum(all_gather_list(n_acc))
    n_label = sum(all_gather_list(n_label))
    tot_time = time()-st
    # val_loss /= n_word
    loss = edit_dist / n_word
    avg_acc = n_acc/n_label*100
    val_log = {'edit_dist_loss': loss,
               'edit_dist': edit_dist,
               'edit_distn_word': n_word,
               'tok_per_s': n_word/tot_time}
    LOGGER.info(f"validation finished in {int(tot_time)} seconds, "
                f"Edit_dist_loss: {loss:.2f}"
                f"Avg_acc: {avg_acc:.2f}")
    return val_log


@torch.no_grad()
def edit_distance_ioc_wocap(model, val_loader):
    LOGGER.info("start running IOC edit distance validation...")
    edit_dist = 0
    n_word = 0
    n_label = 0
    ...
    for i, samples in enumerate(dataset.image_ordering_one_cluster):
        inputs, ordering = dataset.get_one_cluster(samples)
        # inputs = list(zip(*inputs))
        batch = ioc_collate_wo_cap(list(inputs))
        batch = move_to_cuda(batch)
        ioc_scores = model(batch, task='wocap', compute_loss=False)
        pred_labels = ioc_scores.new_zeros(ioc_scores.shape)
        pred_labels[ioc_scores>0] = 1
        pred_ordering = label2ordering(pred_labels.squeeze().int(), len(ordering))
        edit_dist += levenshtein(''.join([str(i) for i in range(len(ordering))]), ''.join([str(i) for i in pred_ordering]))
...
@torch.no_grad()
def validate_eoc(model, val_loader):
    LOGGER.info("start running VCC validation...")
    val_loss = 0
    tot_score = 0
    n_ex = 0
    st = time()
    for i, batch in enumerate(val_loader):
        scores = model(batch, task='eoc', compute_loss=False)
        targets = batch['targets']
        loss = F.cross_entropy(scores, targets, reduction='sum')
        val_loss += loss.item()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', default='pretrain', choices=['pretrain', 'vcr'],
                        help='which task to pretrain')
