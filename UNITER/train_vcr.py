...
def clear_check_point(state_dict):
    new_state_dict = {}
    for k, v in state_dict.items():
        new_k = k if 'module' not in k else '.'.join(k.split('.')[1:])
...
def main(opts):
    ...
    if opts.checkpoint and opts.checkpoint_from == "pretrain":
        if '.model' in opts.checkpoint:
            checkpoint = torch.load(opts.checkpoint)['state_dict']
            checkpoint = clear_check_point(checkpoint)
        else:
    ...
    if opts.checkpoint_from == "vcr_pretrain" and opts.rank == 0:
        if '.model' in opts.checkpoint:
            checkpoint = torch.load(opts.checkpoint)['state_dict']
            checkpoint = clear_check_point(checkpoint)
        else:
    ...
    if rank == 0:
        ...
        if not os.path.exists(join(opts.output_dir, 'ckpt')):
